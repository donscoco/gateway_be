...
func (t *App) Find(c *gin.Context, tx *gorm.DB, search *App) (*App, error) {
	model := &App{}
	err := tx.Set("context", bl.GetGinTraceContext(c)).Where(search).Find(model).Error
	return model, err
}

func (t *App) Save(c *gin.Context, tx *gorm.DB) error {
	if err := tx.Set("context", bl.GetGinTraceContext(c)).Save(t).Error; err != nil {
		return err
	}
	return nil
}

func (t *App) APPList(c *gin.Context, tx *gorm.DB, params *model.APPListInput) ([]App, int64, error) {
	...
	query := tx.Set("context", bl.GetGinTraceContext(c)).
		Table(t.TableName()).
		Select("*").
		Where("is_delete=?", 0)
