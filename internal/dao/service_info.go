...
func (t *ServiceInfo) GroupByLoadType(c *gin.Context, tx *gorm.DB) ([]model.DashServiceStatItemOutput, error) {
	list := []model.DashServiceStatItemOutput{}
...
func (t *ServiceInfo) PageList(c *gin.Context, tx *gorm.DB, param *model.ServiceListInput) ([]ServiceInfo, int64, error) {
	...
	query := tx.Set("context", bl.GetGinTraceContext(c)).Table(t.TableName()).Where("is_delete=0")
	if param.Info != "" {
	...
	err = query.Limit(param.PageSize).Offset(offset).Count(&total).Error
	if err != nil {
...
func (t *ServiceInfo) Find(c *gin.Context, tx *gorm.DB, search *ServiceInfo) (*ServiceInfo, error) {
	out := &ServiceInfo{}
	err := tx.Set("context", bl.GetGinTraceContext(c)).Where(search).Find(out).Error
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (t *ServiceInfo) Save(c *gin.Context, tx *gorm.DB) error {
	return tx.Set("context", bl.GetGinTraceContext(c)).Save(t).Error
}
