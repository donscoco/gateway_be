package tcp_proxy_service

import (
	"context"
	"fmt"
	"github.com/donscoco/gateway_be/internal/dao"
	"github.com/donscoco/gateway_be/internal/middleware/proxy_middleware/tcp_proxy_middleware"
	"github.com/donscoco/gateway_be/internal/module/service_manager"
	"github.com/donscoco/gateway_be/internal/reverse_proxy"
	"github.com/donscoco/gateway_be/pkg/tcp_server"
	"log"
	"net"
)
...
func TcpServerRun() {
	serviceList := service_manager.ServiceManagerHandler.GetTcpServiceList()
	for _, serviceItem := range serviceList {
		tempItem := serviceItem
		go func(serviceDetail *dao.ServiceDetail) {
			addr := fmt.Sprintf(":%d", serviceDetail.TCPRule.Port)
			rb, err := service_manager.LoadBalancerHandler.GetLoadBalancer(serviceDetail)
			if err != nil {
