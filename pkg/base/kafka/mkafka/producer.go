...
var logger = iron_log.NewLogger("mq-kafka")
...
func CreateProducer(Brokers []string, config map[string]string) (p *KafkaProducer, err error) {
	p = new(KafkaProducer)
	...
	p.producer, err = sarama.NewAsyncProducer(Brokers, p.saramaConfig)
	if err != nil {
		return nil, err
	}
